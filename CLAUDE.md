...
| **[x]** Fotoğraf/ekran görüntüsü hatası: ilk çekimde sağ üstteki sembol vb. öğeler görüntüye girmiyor, ikinci basışta hepsi giriyor -- kök neden bulundu: `_arayuzSembol()` simgeyi `onload` ile "hazır" sayıyordu, ama WebKit'te SVG data-URI'lerin çözümlemesi `onload`'dan SONRA bitebiliyor (off-main-thread), yani ilk çekimde simge boş çıkıyordu; ikinci çekimde onbellekten geldiği için sorun kayboluyordu. `onload` yerine `HTMLImageElement.decode()` kullanılıyor artık (görsel gerçekten çizilebilir olmadan söz çözülmüyor). Chromium'da bu yarış hiç üretilemediği için (bu ortamda WebKit kurulamıyor, yalnızca CI'da çalışıyor) yerel kırmızı bir ölçüm alınamadı -- düzeltme WebKit'in belgeli davranışına dayanıyor, gerçek telefonda doğrulanmalı | *"ilk photo çekince sağ üstteki sembol vs bişeyleri almıyor bir daha photoya basınca herşeyi alıyor."* |
